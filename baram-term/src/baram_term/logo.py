...
로고 글자는 7x7 도트 비트맵으로 정의하고, 반쪽 블록 문자(▀ ▄ █)로 셀 1칸 = 가로 1도트 x 세로 2도트로 그린다.
- D2Coding 셀은 가로:세로가 약 8:19 라 반쪽 블록 도트가 거의 정사각형이고, 글자 비율(약 0.85)이 원본 로고와 비슷하다.
- 글자 높이는 홀수(7): 가운데 도트 줄이 있어야 B/A/R 의 가운데 가로획이 정확히 가운데에 온다
  (6도트일 때는 반 도트 위로 보였다). 그림자 1도트를 더한 8도트가 반쪽 블록 4줄에 딱 맞는다.
  그림자 없이 쓸 때 전체 높이가 홀수면 마지막 줄이 반만 차서 R 다리 같은 모양이 뭉개졌다.
