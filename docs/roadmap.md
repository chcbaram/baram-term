...
| baram-term BLE (2026-09-20) | `ble://이름[#꼬리표]` 로 Nordic UART Service 보드에 연결, 포트 설정 창의 종류(시리얼/BLE)와 장치 검색(NUS 만, 가까운 순), 포트 메뉴에서 켜고 끄기(기본 꺼짐), bleak 선택 설치 + 릴리스 빌드에는 포함, 상태줄 `BLE MTU 247`, ctl status 의 `kind`/`mtu` ([ble.md](ble.md)) |
