...
| baram-term 메모 탭 | 오른쪽 패널을 탭으로 (HEX + 메모 최대 8개): 여러 줄 편집, 자동 저장(`notes.json`), 탭 추가/이름 바꾸기/삭제, 줄·선택·전체 보내기(프롬프트 대기 또는 고정 간격, `#wait 밀리초`, 중지), .txt/.json 내보내기·가져오기, 보기 메뉴 `오른쪽 패널 ▸ HEX 보기 / 메모 보기` |
