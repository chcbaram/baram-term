...
| baram-term 매크로 막대 (F1~F12 키/클릭 전송, 오른쪽 클릭 수정, hover 툴팁, 설정 저장) | 코드 완료, 막대 모양 확인받음, **실제 창에서 클릭/툴팁 확인 대기** |
| 다음 | 매크로 막대 확인 → HEX 분할 보기, 배포 |
| 테스트 | retro-ui 238개, baram-term 186개 (+ 로컬 장치 기록이 있으면 재생 테스트) |
...
- **확인 대기**: 실제 창에서 매크로 막대 (오른쪽 클릭, 툴팁 뜨는 시간, F 키 전송, 설정 파일에 남는지)
- 아직 실제로 확인하지 못한 것 (이전부터)
  - 실제 창에서 손가락 커서(링크/상태줄 항목)와 ↕ 커서(터미널/그래프 경계) 모양
  - 연결된 상태에서 상태줄로 속도만 바꿀 때 실제 보드가 끊기지 않는지
- 작업하다 찾은 문제 세 가지는 고치지 않고 적어 두었다:
  [roadmap.md 의 "찾아 둔 문제"](roadmap.md#찾아-둔-문제-고치기-전).
  특히 1번(부분 갱신에서 줄 끝 한글이 빠짐)은 상태줄에서 눈으로 보인다.
